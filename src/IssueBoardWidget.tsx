...
function IssueForm({ schema, initial, isNew, canDelete, onSave, onDelete, onCancel }: IssueFormProps) {
            ...
            return (
              <div key={field.key} style={s.fieldGroup}>
                ...
                {field.type === 'enum' && field.values ? (
                  ...
                  <input type={field.type === 'user' ? 'text' : 'text'} {...commonProps}
...
function ListView({ issues, schema, onEdit }: ListViewProps) {
...
function KanbanView({ issues, schema, kanbanField, onEdit }: KanbanViewProps) {
  const titleField = schema.fields.find(f => f.key === 'title') ?? schema.fields[0];
...
export function IssueBoardWidget({ widgetApi }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [schemaEvents, issueEvents, plEvents] = await Promise.all([
        ...
        if (pl && userId) {
          ...
          // If we can't check, optimistically allow writes (errors surface on save)
          setCanWrite(true);
        }
      } catch (e) {
      ...
      if (event.type === 'eu.kiefte.issues.schema') {
        ...
        if (c?._deleted) {
          ...
          setIssues(prev => {
            ...
            return [
              ...rest,
              { stateKey: event.state_key!, content: c, sender: event.sender, ts: event.origin_server_ts },
            ];
          });
        }
      }
  ...
  const doSave = useCallback(async (stateKey: string, content: IssueContent) => {
    ...
    setIssues(prev => {
      ...
      return [...rest, { stateKey, content, sender: userId, ts: Date.now() }];
    });
    setEditing(null);
